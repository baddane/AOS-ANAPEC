...
const CATEGORY_META: Record<ConventionCategory, { label: string; bg: string; text: string; icon: string; gradient: string }> = {
  HEBERGEMENT: { label: 'Hôtels & Estivage', bg: 'bg-brand-blue-light', text: 'text-brand-blue-dark font-semibold', icon: '🏨', gradient: 'from-blue-600 to-cyan-500' },
  TRANSPORT: { label: 'Transport & Voyages', bg: 'bg-indigo-50', text: 'text-indigo-700', icon: '🚄', gradient: 'from-indigo-600 to-purple-500' },
  SANTE: { label: 'Santé & Optique', bg: 'bg-rose-50', text: 'text-rose-700', icon: '👓', gradient: 'from-rose-600 to-pink-500' },
  BANQUE_ASSUR: { label: 'Banque & Assurance', bg: 'bg-amber-50', text: 'text-amber-700', icon: '💳', gradient: 'from-amber-600 to-orange-500' },
  EDUCATION: { label: 'Éducation & Crèches', bg: 'bg-violet-50', text: 'text-violet-700', icon: '🎓', gradient: 'from-violet-600 to-indigo-500' },
  SOURIRE: { label: 'Loisirs & Bien-être', bg: 'bg-brand-gold-light', text: 'text-brand-gold-dark font-semibold', icon: '💪', gradient: 'from-yellow-500 to-amber-500' }
};
...
export default function ConventionsDirectory({ currentUser, conventions }: ConventionsDirectoryProps) {
  ...
  const handleShare = useCallback(async () => {
    ...
    const text = `Convention AOS ANAPEC: ${selectedConvention.title} - ${selectedConvention.discountValue} chez ${selectedConvention.partnerName}`;
    try {
    ...
    <div className="space-y-6" id="conventions-directory-container">
      ...
      <div className="bg-gradient-to-br from-brand-blue-deep to-brand-blue rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-lg shadow-brand-blue/10">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <h2 className="font-display text-2xl md:text-3xl font-bold tracking-tight">Accords Sociaux & Remises Négociées</h2>
          <p className="mt-2 text-sm text-brand-blue-light/90 leading-relaxed">
            Profitez de réductions exceptionnelles négociées par l'AOS de l'ANAPEC pour vous, vos conjoints et enfants. Cliquez sur une convention pour explorer ses termes en détail.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-xs border border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-80">
          ...
          <input type="text" placeholder="Rechercher un hôtel, transport, opticien..." value={search} onChange={(e) => setSearch(e.target.value)}
        ...
        <div className="w-full flex gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
          <button onClick={() => setSelectedCategory('ALL')} className={`px-3 py-2 text-xs font-semibold rounded-xl shrink-0 transition-all cursor-pointer ${selectedCategory === 'ALL' ? 'bg-brand-blue text-white shadow-sm' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>Tous les accords</button>
      ...
      <div className="flex justify-between items-center px-1">
        <p className="text-xs text-slate-400 font-medium">{filtered.length} {filtered.length > 1 ? 'partenaires trouvés' : 'partenaire trouvé'}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              <div className={`p-5 space-y-4 ${rich ? 'cursor-pointer' : ''}`} onClick={() => rich ? handleOpenDetail(conv) : undefined}>
                <div className="flex justify-between items-start gap-2">
                  ...
                  {conv.highlighted && (
                    <span className="bg-brand-gold-accent text-brand-gold-dark text-[10px] px-2 py-0.5 rounded-md font-bold uppercase tracking-wider animate-pulse">
                      <Star className="w-3 h-3 inline -mt-0.5 mr-0.5" />Recommandé
                    </span>
                  )}
                </div>
                ...
                {rich && (
                  <div className="flex gap-2">
                    {conv.establishments && <span className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 text-[10px] font-bold rounded-lg"><Building2 className="w-3 h-3" />{conv.establishments.length} établissements</span>}
                    {conv.articles && <span className="inline-flex items-center gap-1 px-2 py-1 bg-violet-50 text-violet-600 text-[10px] font-bold rounded-lg"><FileText className="w-3 h-3" />{conv.articles.length} articles</span>}
                  </div>
                )}
                <div className="space-y-1.5 pt-3 border-t border-slate-100 text-[11px] text-slate-500">
                  ...
                  <div className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-slate-400 shrink-0" /><span>Valide jusqu'au: {new Date(conv.validityDate).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' })}</span></div>
                </div>
              </div>
              <div className="p-4 bg-slate-50/70 border-t border-slate-100 flex gap-2">
                {rich && <button onClick={() => handleOpenDetail(conv)} className="inline-flex justify-center items-center gap-1.5 px-3 py-2 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold rounded-lg transition-all cursor-pointer"><Eye className="w-3.5 h-3.5" />Explorer</button>}
                <button onClick={(e) => { e.stopPropagation(); handleCreateVoucher(conv); }} className="flex-1 inline-flex justify-center items-center gap-1.5 px-3 py-2 bg-brand-blue hover:bg-brand-blue-dark text-white text-xs font-semibold rounded-lg transition-all cursor-pointer shadow-xs"><Ticket className="w-3.5 h-3.5" />Générer mon bon d'accord</button>
              </div>
            </div>
          );
        })}
        {filtered.length === 0 && (
          <div className="col-span-full bg-white p-12 text-center rounded-2xl border border-slate-100 space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto">Nous n'avons trouvé aucun partenariat actif correspondant à vos critères de recherche.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/70 backdrop-blur-sm flex items-stretch justify-center">
          <div className="bg-white w-full max-w-4xl flex flex-col overflow-hidden md:my-4 md:rounded-2xl md:mx-4 shadow-2xl">
            ...
            <div className={`bg-gradient-to-r ${CATEGORY_META[selectedConvention.category].gradient} p-5 text-white relative overflow-hidden shrink-0`}>
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                  <button onClick={() => setShowDetail(false)} className="inline-flex items-center gap-1 text-white/80 hover:text-white text-xs font-semibold transition-colors cursor-pointer"><ArrowLeft className="w-4 h-4" /> Retour</button>
                  <div className="flex gap-2">
                    {selectedConvention.signatureDate && <span className="px-2.5 py-1 bg-white/15 backdrop-blur rounded-lg text-[10px] font-bold flex items-center gap-1"><Calendar className="w-3 h-3" /> Signée le {new Date(selectedConvention.signatureDate).toLocaleDateString('fr-FR')}</span>}
                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 ${isActive(selectedConvention) ? 'bg-emerald-400/20 text-emerald-100' : 'bg-red-400/20 text-red-200'}`}>
                      {isActive(selectedConvention) ? <><CheckCircle2 className="w-3 h-3" /> Active</> : <><Clock className="w-3 h-3" /> Expirée</>}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <div className="bg-white/15 backdrop-blur rounded-xl px-4 py-2">
                    <p className="text-[10px] text-white/60 font-bold uppercase tracking-wider">Avantage</p>
                    <p className="text-base font-black">{selectedConvention.discountValue}</p>
                  </div>
                  {selectedConvention.duration && <div className="bg-white/15 backdrop-blur rounded-xl px-4 py-2"><p className="text-[10px] text-white/60 font-bold uppercase tracking-wider">Durée</p><p className="text-sm font-bold">{selectedConvention.duration}</p></div>}
                  {selectedConvention.establishments && <div className="bg-white/15 backdrop-blur rounded-xl px-4 py-2"><p className="text-[10px] text-white/60 font-bold uppercase tracking-wider">Réseau</p><p className="text-sm font-bold">{selectedConvention.establishments.length} établissements</p></div>}
                </div>
              </div>
            </div>
            {/* Tabs */}
            <div className="border-b border-slate-100 bg-slate-50/50 px-5 flex gap-1 shrink-0 overflow-x-auto scrollbar-none">
              {[
                { id: 'APERCU' as DetailTab, label: 'Aperçu', icon: <Eye className="w-3.5 h-3.5" /> },
                ...(selectedConvention.articles?.length ? [{ id: 'ARTICLES' as DetailTab, label: `Articles (${selectedConvention.articles.length})`, icon: <FileText className="w-3.5 h-3.5" /> }] : []),
                ...(selectedConvention.establishments?.length ? [{ id: 'RESEAU' as DetailTab, label: `Réseau (${selectedConvention.establishments.length})`, icon: <Building2 className="w-3.5 h-3.5" /> }] : []),
                ...(selectedConvention.requiredDocuments?.length ? [{ id: 'DOCUMENTS' as DetailTab, label: 'Documents', icon: <Shield className="w-3.5 h-3.5" /> }] : []),
              ].map(tab => (
              ...
              {detailTab === 'APERCU' && (
                <div className="space-y-6">
                  ...
                  {selectedConvention.beneficiaries && (
                    <div>
                      <h3 className="font-display font-bold text-sm text-slate-800 mb-3 flex items-center gap-2"><div className="p-1.5 bg-emerald-50 rounded-lg"><Users className="w-4 h-4 text-emerald-600" /></div>Bénéficiaires Éligibles</h3>
                  ...
                  {selectedConvention.coveredServices && (
                    <div>
                      <h3 className="font-display font-bold text-sm text-slate-800 mb-3 flex items-center gap-2"><div className="p-1.5 bg-rose-50 rounded-lg"><Heart className="w-4 h-4 text-rose-600" /></div>Prestations Couvertes</h3>
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                    <div className="flex justify-between items-center mb-2">
                      <p className="text-xs font-semibold text-slate-600">Progression de lecture</p>
                      <div className="flex items-center gap-3">
                        <p className="text-xs font-bold text-brand-blue">{expandedArticles.size}/{selectedConvention.articles.length} articles consultés</p>
                        <button onClick={expandAllArticles} className="text-[10px] font-bold text-brand-blue hover:underline cursor-pointer">{expandedArticles.size === selectedConvention.articles.length ? 'Tout replier' : 'Tout déployer'}</button>
                      </div>
                    </div>
                        ...
                        <div key={article.number} className={`border rounded-xl overflow-hidden transition-all ${isOpen ? 'border-brand-blue/30 shadow-sm' : 'border-slate-100'}`}>
                          <button onClick={() => toggleArticle(article.number)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-slate-50 transition-colors cursor-pointer">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 text-[9px] font-bold rounded-md">Art. {article.number}</span>
              ...
              {detailTab === 'RESEAU' && selectedConvention.establishments && (
                <div className="space-y-4">
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400"><Search className="w-4 h-4" /></div>
                    <input type="text" placeholder="Rechercher par nom, code ou ville..." value={networkSearch} onChange={(e) => setNetworkSearch(e.target.value)}
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <span className="px-3 py-1.5 bg-brand-blue-light text-brand-blue-dark text-xs font-bold rounded-lg">{establishmentsByCity.length} villes</span>
                    <span className="px-3 py-1.5 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-lg">{totalFilteredEstablishments} établissements</span>
                  </div>
                  <div className="space-y-4">
                  ...
                  {establishmentsByCity.length === 0 && <div className="text-center py-8 text-slate-400"><Building2 className="w-8 h-8 mx-auto mb-2 opacity-50" /><p className="text-xs font-semibold">Aucun établissement ne correspond à votre recherche</p></div>}
                </div>
              )}
              {/* DOCUMENTS */}
              {detailTab === 'DOCUMENTS' && selectedConvention.requiredDocuments && (
                <div className="space-y-4">
                  <div className="bg-amber-50 border border-amber-100 rounded-xl p-4"><p className="text-xs text-amber-700 font-semibold flex items-center gap-2"><Shield className="w-4 h-4" />Documents requis pour accéder aux prestations de cette convention</p></div>
                  <div className="space-y-2">
            ...
            <div className="shrink-0 p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
              <button onClick={() => { setShowDetail(false); handleCreateVoucher(selectedConvention); }}
                ...
                <Ticket className="w-4 h-4" />Générer mon bon d'accord
              </button>
              <button onClick={handleShare} className="px-4 py-2.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold rounded-lg transition-all cursor-pointer inline-flex items-center gap-1.5">
                {copied ? <><Check className="w-3.5 h-3.5 text-emerald-500" />Copié !</> : <><Copy className="w-3.5 h-3.5" />Partager</>}
              </button>
              <button onClick={() => setShowDetail(false)} className="px-4 py-2.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold rounded-lg transition-all cursor-pointer">Fermer</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-100 flex flex-col justify-between">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <span className="text-xs font-bold text-brand-blue flex items-center gap-1.5"><Ticket className="w-4 h-4" />VOTRE BON D'OFFRE CONVENTIONNÉ</span>
            ...
            <div className="p-6 space-y-6" id="printable-voucher-document">
              <div className="flex justify-between items-start border-b-2 border-slate-100 pb-4">
                ...
                <div className="text-right"><span className="text-[10px] font-mono font-bold bg-slate-100 text-slate-800 px-2 py-1 rounded-md">{voucherCode}</span><p className="text-[9px] text-slate-400 mt-1">Généré le: {new Date().toLocaleDateString('fr-FR')}</p></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                ...
                <div className="p-3 bg-slate-50 rounded-xl space-y-1">
                  ...
                  <p className="text-[11px] text-brand-blue-dark font-bold flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-brand-blue" /> Cotisation Active</p>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-100">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="text-left font-mono"><p className="text-[9px] text-slate-400 uppercase font-semibold">Validation instantanée</p><p className="text-[10px] font-bold text-brand-blue-deep">CERTIFIÉ CONFORME</p></div>
                </div>
                <div className="text-right text-[10px] text-slate-400">
                  ...
                  <div className="mt-1 h-8 w-24 bg-brand-blue-light border-2 border-dashed border-brand-blue/20 rounded-md flex items-center justify-center text-[8px] text-brand-blue font-bold tracking-widest uppercase select-none pointer-events-none transform -rotate-3">AOS QUALIFIÉ</div>
                </div>
              </div>
            </div>
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
              <button onClick={() => window.print()} className="flex-1 inline-flex justify-center items-center gap-1.5 px-4 py-2 bg-slate-700 hover:bg-slate-800 text-white text-xs font-semibold rounded-lg transition-all cursor-pointer"><Printer className="w-3.5 h-3.5" />Imprimer le Bon d'Accord</button>
              <button onClick={() => setShowVoucher(false)} className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold rounded-lg transition-all cursor-pointer">Fermer</button>
            </div>
          </div>
        </div>
      )}
